...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/Status-第四阶段完成！-brightgreen" alt="Status">
</div>
...
### 🌐 第五阶段：真实场景推理（Real-World Inference）

- **挑战任务**：脱离 MNIST 测试集，**用训练好的模型去识别一张真实拍摄/手写的数字照片**，检验模型在真实环境下的泛化能力与鲁棒性。
- **核心工作**：
  1. **模型部署**：编写 `predict_photo.py`，利用 `save_and_load.py` 加载已经训练好的 `lenet_params.pkl`。
  2. **图片预处理**：将任意大小的彩色/灰度照片，通过 **灰度化 → Resize（28×28） → 归一化（0~1） → Reshape 为 (1, 1, 28, 28)**，转化为模型能“吃”的标准 4D 张量。
  3. **模型推理**：传入 `network.predict()`，输出预测概率，取最大概率的类别作为最终结果。
  4. **实战检验**：用自己手写的数字、网上的图片或手机拍的照片进行测试，分析模型在真实场景（如相机噪点、歪斜、涂抹、反色）下的表现。
- **进阶优化（可选）**：如果在实拍照片上识别率不佳（常见问题：MNIST 是白底黑字，实拍是黑底白字），可以尝试加入**反色处理**或**画框定位中心**等传统图像处理技巧。
- **任务进度**：进行中（等待部署实现）
