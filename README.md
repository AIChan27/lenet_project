...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/Status-项目圆满结束！-brightgreen" alt="Status">
</div>
...
### 🧪 第四阶段：现代技术注入
- **手写批归一化（Batch Normalization）**：在 `layers.py` 中手写 BN 类，插入到卷积层和全连接层之间，尝试将准确率从 **96% 冲击到 98%+**。
...
# 早停耐心值改为20
patience = 20
# 优化学习率衰减时间，把学习率衰减推迟到 50 轮（等模型稳住了，再降学习率微调）
if epoch != 0 and epoch % 50 == 0:
    self.optimizer.lr *= 0.1
    print(f"第 {epoch} 个 epoch，学习率已衰减为 {self.optimizer.lr}")
</pre>
...
## 🧱 LeNet-5 标准维度流（第四阶段优化后）

| 层名称 | 操作 | 输入维度 | 输出维度 | 参数/备注 |
| :--- | :--- | :--- | :--- | :--- |
| **C1** | 卷积 + **Batch Norm** + ReLU | (N, 1, 28, 28) | (N, 6, 24, 24) | 6个 5x5，步幅1，无填充 |
| **S2** | 池化 | (N, 6, 24, 24) | (N, 6, 12, 12) | 窗口 2x2，步幅2 |
| **C3** | 卷积 + **Batch Norm** + ReLU | (N, 6, 12, 12) | (N, 16, 8, 8) | 16个 5x5，步幅1 |
| **S4** | 池化 | (N, 16, 8, 8) | (N, 16, 4, 4) | 窗口 2x2，步幅2 |
| **F5** | 展平 + Affine + **Batch Norm** + ReLU | (N, 256) | (N, 120) | 全连接，BN 在激活前 |
| **F6** | Affine + **Batch Norm** + ReLU | (N, 120) | (N, 84) | 全连接，BN 在激活前 |
| **Out** | Affine + Softmax | (N, 84) | (N, 10) | 输出层，**不加 BN** |
